...
CONFIG: Dict[str, Any] = {
    ...
    "LOITERING_TIME_SEC": 20.0,
    "RUNNING_SPEED_THRESHOLD": 85.0,
    "RUNNING_MIN_DURATION_SEC": 1.5,
    ...
    "COUNT_LINE_Y": 360,
    "ENTRY_EXIT_COUNTER_ENABLED": True,
        ...
        if self.enabled:
            try:
                ...
                if solutions is None:
                    raise AttributeError("mediapipe.solutions is unavailable")
                ...
                self.enabled = False
                print(f"[WARN] MediaPipe hand detector disabled: {e}")
    ...
    def __init__(self, cfg=None):
        ...
        if self.enabled:
            try:
                ...
                self.enabled = False
                print(f"[WARN] MediaPipe pose detector disabled: {e}")
...
class BehaviorAnalyzer:
    ...
    def update(self, tracked: Dict[int, Tuple[int, int]]) -> List[tuple]:
        ...
        for oid, c in tracked.items():
            ...
            self._update_loitering_state(h, c)
    ...
    def _update_loitering_state(self, h, c):
        x1, y1, x2, y2 = self.cfg.get("LOITERING_ZONE", (0, 0, 0, 0))
        inside = x1 <= c[0] <= x2 and y1 <= c[1] <= y2
        if inside and h.get("loitering_entered_at") is None:
            h["loitering_entered_at"] = self.current_time
        elif not inside:
            h["loitering_entered_at"] = None

    def _loitering(self, h):
        entered = h.get("loitering_entered_at")
        if entered is None:
            return None
        dur = self.current_time - entered
        threshold = float(self.cfg.get("LOITERING_TIME_SEC", 20.0))
        if dur >= threshold:
            return f"inside loitering zone for {dur:.1f}s"
        return None

    def _running(self, h):
        if len(h["speeds"]) < 5: return None
        speed_thr = float(self.cfg.get("RUNNING_SPEED_THRESHOLD", 85.0))
        min_dur = float(self.cfg.get("RUNNING_MIN_DURATION_SEC", 1.5))
        ts = list(h["timestamps"]); sp = list(h["speeds"])
        fast_dur = 0.0
        fast_speeds = []
        for i in range(len(sp) - 1, 0, -1):
            if sp[i] >= speed_thr:
                fast_dur += max(0.0, ts[i] - ts[i - 1])
                fast_speeds.append(sp[i])
            else:
                break
        if fast_dur >= min_dur:
            avg = float(np.mean(fast_speeds)) if fast_speeds else 0.0
            return f"running {fast_dur:.1f}s (avg {avg:.1f}px/s)"
        return None
...
class CrowdIntelligence:
    def __init__(self, cfg=None):
        ...
        self._prev_positions: Dict[int, Tuple[Tuple[int, int], float]] = {}
        self._last_event: Dict[str, float] = defaultdict(float)
    ...
    def update(self, tracked: Dict[int, Tuple[int, int]], frame_size=(720, 1280)):
        ...
        now = time.time()
        avg_speed = self._average_speed(tracked, now)
        ...
        if len(positions) >= thr:
            ...
            for row in range(cong_grid[0]):
                for col in range(cong_grid[1]):
                    ...
                    if cnt >= thr:
                        if self._cooldown("CONGESTION", 8.0):
                            events.append(("CONGESTION", f"ZONE_{row}_{col}", 1.0,
                                           f"{cnt} people in zone"))

        evac_min = int(self.cfg.get("EVAC_MIN_PEOPLE", 4))
        evac_thr = float(self.cfg.get("EVAC_AVG_SPEED_THRESHOLD", 25.0))
        if len(positions) >= evac_min and avg_speed >= evac_thr and self._cooldown("EVACUATION_ALERT", 12.0):
            events.append(("EVACUATION_ALERT", "SYSTEM", min(1.0, avg_speed / max(evac_thr, 1.0)),
                           f"{len(positions)} people moving fast; avg speed {avg_speed:.1f}px/s"))
        return events

    def _average_speed(self, tracked, now):
        speeds = []
        for oid, c in tracked.items():
            prev = self._prev_positions.get(oid)
            if prev:
                pc, ts = prev
                dt = max(1e-3, now - ts)
                speeds.append(math.hypot(c[0] - pc[0], c[1] - pc[1]) / dt)
            self._prev_positions[oid] = (c, now)
        for oid in list(self._prev_positions.keys()):
            if oid not in tracked:
                self._prev_positions.pop(oid, None)
        return float(np.mean(speeds)) if speeds else 0.0

    def _cooldown(self, key, seconds):
        now = time.time()
        if now - self._last_event.get(key, 0.0) < seconds:
            return False
...
class VisionSystem:
    ...
    def _init_state(self):
        ...
        self._line_counter = {"in": 0, "out": 0}
        self._line_last_side: Dict[int, int] = {}
        self._ppe_missing_since: Dict[str, float] = {}
        self._attendance_badges: Dict[str, str] = {}
    ...
    def add_embedding_for_name(self, person_name: str, embedding: np.ndarray) -> bool:
        ...
        dup_cfg = self.cfg.get("ENROLLMENT_DUPLICATE_GUARD", {})
        if dup_cfg.get("ENABLED", True) and person_name not in self.face_db and self.face_db:
            existing_name, sim, reason = self.recognize_face(embedding)
            if (existing_name not in ("UNKNOWN", "SPOOF")
                    and not str(existing_name).startswith("STRANGER_")):
                th = self.face_db.get(existing_name, {}).get("threshold", self.cfg["FACE_RECOG_THRESHOLD"])
                margin = float(dup_cfg.get("MARGIN", 0.03))
                if sim >= th + margin:
                    print(f"[ENROLL-guard] '{person_name}' looks like existing identity '{existing_name}' "
                          f"(sim={sim:.3f}, threshold={th:.3f}). Enrollment skipped.")
                    return False
        if person_name not in self.face_db:
            self.face_db[person_name] = {"embeddings": [],
                                         "threshold": self.cfg["FACE_RECOG_THRESHOLD"]}
    ...
    def enroll_from_image_path(self, path: str, person_name: str, db: EventDatabase = None) -> bool:
        if not path or not os.path.exists(path):
            print(f"[ENROLL-file] Path not found: {path}")
            return False
        paths = []
        ...
        for img_path in paths:
            ...
            if img is None:
                print(f"[ENROLL-file] Could not read {img_path}")
                continue
            faces = self.face_analyzer.detect(img)
            if len(faces) != 1:
                print(f"[ENROLL-file] Skip {img_path}: expected 1 face, got {len(faces)}")
                continue
            x1, y1, x2, y2 = self.face_analyzer.get_bbox(faces[0])
            crop = img[max(0, y1):min(img.shape[0], y2), max(0, x1):min(img.shape[1], x2)]
            if crop.size:
                ok, score = self.quality_scorer.is_acceptable(crop)
                if not ok:
                    print(f"[ENROLL-file] Skip {img_path}: low quality {score:.0f}")
                    continue
            if self.add_embedding_for_name(person_name, self.face_analyzer.get_embedding(faces[0])):
                added += 1
        if added and db is not None:
            db.upsert_person(person_name)
            db.log_audit("ENROLL_FROM_IMAGE_FILE", person_name, {"path": path, "added": added})
        print(f"[ENROLL-file] {person_name}: added {added} embedding(s)")
        return added > 0
    ...
    def _draw_face_box(self, frame, x1, y1, x2, y2, name, conf, oid, is_real):
        ...
        badge = self._attendance_badges.get(name)
        if badge and name not in ("UNKNOWN", "SPOOF") and not str(name).startswith("STRANGER_"):
            by = min(frame.shape[0] - 8, y2 + 18)
            cv2.putText(frame, badge, (x1, by), cv2.FONT_HERSHEY_SIMPLEX,
                        0.45, (0, 255, 255), 1)

    def set_attendance_badges(self, badges: Dict[str, str]):
        self._attendance_badges = badges or {}
    ...
    def _check_line_crossings(self, tracked) -> List[tuple]:
        if not self.cfg.get("ENTRY_EXIT_COUNTER_ENABLED", True):
            return []
        line_y = self.cfg.get("COUNT_LINE_Y", 360)
        events = []
        for oid, c in tracked.items():
            side = -1 if c[1] < line_y else 1
            prev = self._line_last_side.get(oid)
            self._line_last_side[oid] = side
            if prev is None or prev == side:
                continue
            if prev < side:
                self._line_counter["in"] += 1
                direction = "IN"
            else:
                self._line_counter["out"] += 1
                direction = "OUT"
            events.append(("LINE_CROSSING", f"ID_{oid}", 1.0,
                           f"ID_{oid} crossed count line {direction}"))
        for oid in list(self._line_last_side.keys()):
            if oid not in tracked:
                self._line_last_side.pop(oid, None)
        return events

    def _check_ppe_violations(self, tracked, object_detections) -> List[tuple]:
        cfg = self.cfg.get("PPE_COMPLIANCE", {})
        if not cfg.get("ENABLED", False):
            return []
        required = {x.lower() for x in self.cfg.get("PPE_REQUIRED_OBJECTS", set())}
        if not required:
            return []
        now = time.time()
        events = []
        person_dets = [d for d in object_detections if d.get("class_name") == "person"]
        ppe_dets = [d for d in object_detections if str(d.get("class_name", "")).lower() in required
                    or str(d.get("category", "")).lower() == "ppe"]
        for oid, c in tracked.items():
            person = self._nearest_person_detection(c, person_dets)
            if not person:
                continue
            missing = []
            for item in required:
                has_item = any(str(d.get("class_name", "")).lower() == item
                               and self._bbox_overlap_ratio(person["bbox"], d.get("bbox")) >= cfg.get("MIN_OVERLAP", 0.10)
                               for d in ppe_dets)
                if not has_item:
                    missing.append(item)
            if not missing:
                self._ppe_missing_since.pop(str(oid), None)
                continue
            key = f"{oid}:{','.join(sorted(missing))}"
            self._ppe_missing_since.setdefault(key, now)
            if now - self._ppe_missing_since[key] >= cfg.get("CONFIRM_SECONDS", 4.0):
                cooldown_key = f"ppe:{key}"
                if self._event_cooldown.allowed(cooldown_key, cfg.get("COOLDOWN_SEC", 30.0)):
                    events.append(("PPE_VIOLATION", f"ID_{oid}", 1.0,
                                   f"Missing required PPE: {', '.join(sorted(missing))}"))
        return events

    def _nearest_person_detection(self, centroid, person_dets):
        best, best_d = None, float("inf")
        for d in person_dets:
            ...
            if dist < best_d:
                best, best_d = d, dist
        return best if best_d < 180 else None
    ...
    def _draw_ui(self, frame, events, tracked, obj_dets):
        ...
        if self.cfg.get("SHOW_COUNT_LINE", False):
            ...
            cv2.putText(frame, f"IN:{self._line_counter['in']} OUT:{self._line_counter['out']}",
                        (10, min(h - 15, ly + 22)), cv2.FONT_HERSHEY_SIMPLEX,
                        0.55, (255, 255, 0), 2)
        ...
        hud_y = 90
        cv2.putText(frame, f"Alerts:{sum(1 for e in events if e[0] in _SEVERE_EVENT_TYPES)}",
                    (w - 180, hud_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
        if self.behavior.scores:
            top_id, top_score = max(self.behavior.scores.items(), key=lambda kv: kv[1])
            cv2.putText(frame, f"Top risk ID:{top_id} {top_score:.1f}",
                        (w - 180, hud_y + 24), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
    ...
    def process(self, frame):
        ...
        events.extend(self._check_line_crossings(tracked))
        events.extend(self._check_ppe_violations(tracked, object_detections))
...
class AttendanceManager:
    ...
    def auto_clock_out_stale(self, camera_id: str = "cam_0", location: str = None) -> List[dict]:
        if not self.enabled or not self.cfg.get("AUTO_CLOCKOUT_ENABLED", True):
            return []
        timeout_min = self.cfg.get("DAILY_CLOCKOUT_TIMEOUT_MIN")
        if not timeout_min:
            return []
        self._maybe_rollover()
        cutoff = time.time() - float(timeout_min) * 60.0
        results = []
        for person_name in list(self._today_clocked_in):
            last_seen = self._last_seen.get(person_name)
            if last_seen is None or last_seen >= cutoff:
                continue
            pid = self.db.get_person_id(person_name)
            if pid is None:
                continue
            result = self.db.attendance_clock_out(pid)
            if "clocked_out_at" in result:
                ...
                self.db.log_event("ATTENDANCE_CLOCKOUT", person_id=pid, confidence=1.0,
                                  details={"auto": True, "reason": "stale_recognition_timeout",
                                           "timeout_min": timeout_min},
                                  camera_id=camera_id, location=location, severity=0)
                results.append({"person": person_name, **result})
        return results

    def clock_out_open_sessions_on_shutdown(self) -> List[dict]:
        if not self.enabled or not self.cfg.get("AUTO_CLOCKOUT_ENABLED", True):
            return []
        results = []
        for person_name in list(self._today_clocked_in):
            ...
            if "clocked_out_at" in result:
                self._today_clocked_in.discard(person_name)
                self.db.log_event("ATTENDANCE_CLOCKOUT", person_id=pid, confidence=1.0,
                                  details={"auto": True, "reason": "system_shutdown"}, severity=0)
                results.append({"person": person_name, **result})
        return results
    ...
    def get_badges(self) -> Dict[str, str]:
        badges = {}
        try:
            for row in self.db.attendance_report(days=1):
                name = row["name"]
                if row["clock_out"]:
                    badges[name] = f"OUT {str(row['clock_out'])[11:16]}"
                elif row["clock_in"]:
                    badges[name] = f"IN {str(row['clock_in'])[11:16]}"
        except Exception:
            pass
        return badges
    ...
    def manual_clock_out(self, person_name: str) -> dict:
        ...
        if "clocked_out_at" in r:
            self._today_clocked_in.discard(person_name)
            self._last_seen.pop(person_name, None)
            self.db.log_event("ATTENDANCE_CLOCKOUT", person_id=pid, confidence=1.0,
...
def _export_shutdown_report(db: EventDatabase, vision: VisionSystem, cam_id: str = "cam_0", location: str = None, started_at: float = None) -> Optional[str]:
    try:
        ...
        attendance_csv = os.path.join(report_dir, f"attendance_{ts}.csv")
        with open(attendance_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "name", "date", "clock_in", "clock_out", "work_minutes",
                "late_minutes", "camera_id", "location", "last_seen",
                "recognition_confidence",
            ])
            writer.writeheader()
            for row in people_log:
                writer.writerow({k: row.get(k) for k in writer.fieldnames})

        events_csv = os.path.join(report_dir, f"events_{ts}.csv")
        with open(events_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "timestamp", "event_type", "person", "confidence",
                "severity", "camera_id", "location", "details",
            ])
            writer.writeheader()
            for e in recent_events:
                writer.writerow({
                    ...
                    "confidence": e["confidence"],
                    "severity": e["severity"],
                    "camera_id": e["camera_id"] if "camera_id" in e.keys() else None,
                    "location": e["location"] if "location" in e.keys() else None,
                    "details": e["details_json"],
                })

        print(f"[REPORT] Saved shutdown report -> {path}")
        print(f"[REPORT] Saved CSV exports -> {attendance_csv}, {events_csv}")
        return path
...
def _publish_runtime_heartbeat(runtime_dir: str, engine_status: str, camera_status: str, fps: float, last_error=None):
...
def _publish_runtime_state(runtime_dir: str, vision: VisionSystem, display, cam_id: str, location: str, fps: float, started_at: float, events: list, last_error=None):
    ...
    for face in faces:
        ...
        item = {
            ...
            "visible_seconds": 0,
            "spoof_status": "passed" if face.get("is_real", True) else "suspect",
        }
    ...
    for obj in getattr(vision, "_last_objects_seen", []) or []:
        ...
        object_counts.setdefault(name, {"class_name": name, "count": 0, "confidence": 0.0, "category": obj.get("category")})
        object_counts[name]["count"] += 1
        object_counts[name]["confidence"] = max(object_counts[name]["confidence"], float(obj.get("confidence") or 0))

    active_events = []
    for evt in (events or [])[-10:]:
        try:
            active_events.append({
                "id": f"live_{hash(str(evt)) % 100000}",
                "time": _runtime_now(),
                "type": evt[0],
    ...
    h, w = display.shape[:2] if display is not None else (None, None)
    security_level = "critical" if any(e.get("severity") == "Critical" for e in active_events) else ("attention" if unknown or active_events else "normal")
    _atomic_json_write(os.path.join(runtime_dir, "live_state.json"), {
        ...
        "security": {
            ...
            "message": "Unknown person detected" if unknown else ("Security event active" if active_events else "No active warning"),
...
def _publish_runtime_frame(runtime_dir: str, display, quality=78):
    if display is None:
        return
    tmp = os.path.join(runtime_dir, "latest_frame.tmp.jpg")
    final = os.path.join(runtime_dir, "latest_frame.jpg")
    ok = cv2.imwrite(tmp, display, [int(cv2.IMWRITE_JPEG_QUALITY), int(quality)])
    if ok:
        os.replace(tmp, final)

def _read_pending_commands(runtime_dir: str):
...
def _write_command_result(runtime_dir: str, result: dict):
    ...
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                old = json.load(f)
            results = old.get("results", []) if isinstance(old, dict) else old
        except Exception:
            results = []
    results = [r for r in results if r.get("id") != result.get("id")][-100:]
    results.append(result)
    _atomic_json_write(path, {"results": results})

def _mark_commands_seen(runtime_dir: str, handled_ids: set):
    path = os.path.join(runtime_dir, "commands.json")
...
def _process_runtime_commands(runtime_dir: str, db: EventDatabase, attendance: AttendanceManager, alert_mgr: AlertManager, vision: VisionSystem, last_raw_frame, snapshot_dir: str, cam_id: str, cam_location: str):
    handled = set()
    for cmd in _read_pending_commands(runtime_dir):
        cid = cmd.get("id")
        ctype = cmd.get("type")
        payload = cmd.get("payload") or {}
        result = {"id": cid, "type": ctype, "status": "completed", "completed_at": _runtime_now(), "result": {}, "error": None}
        try:
            if ctype == "save_snapshot":
            ...
            elif ctype == "test_alert":
                result["result"] = {"message": "Test alert requested.", "channels": alert_mgr.test_alert()}
            elif ctype == "manual_clock_in":
                person_id = payload.get("person_id")
                if not person_id:
                    raise RuntimeError("person_id is required.")
                info = db.attendance_clock_in(int(person_id), cam_id, cam_location)
                result["result"] = info
            elif ctype == "manual_clock_out":
                name = payload.get("name")
                person_id = payload.get("person_id")
                if not name and person_id:
                    rows = db.get_known_face_names(limit=500)
                    for r in rows:
                        if int(r["id"]) == int(person_id):
                            name = r["name"]
                            break
                if not name:
                    raise RuntimeError("name or person_id is required.")
                result["result"] = attendance.manual_clock_out(str(name))
            elif ctype == "start_enrollment":
                name = str(payload.get("name") or "").strip()
                if not name:
                    raise RuntimeError("name is required.")
                _set_enrollment_status(runtime_dir, "saving_embeddings", f"Registering visible face as {name}.")
                ok = vision.enroll_best_visible_face(name)
                if ok:
                    db.upsert_person(name)
                    db.log_audit("ENROLL_VISIBLE_FACE_WEB", name, payload)
                    _set_enrollment_status(runtime_dir, "completed", f"{name} enrolled.")
                    result["result"] = {"message": f"{name} enrolled."}
                else:
            ...
            elif ctype == "register_visible_unknown":
                name = str(payload.get("name") or "").strip()
                if not name:
                    raise RuntimeError("name is required.")
                ok = vision.enroll_best_visible_face(name)
                if ok:
                    db.upsert_person(name)
                    result["result"] = {"message": f"{name} enrolled from visible unknown."}
                else:
                    raise RuntimeError("No visible unknown face was available.")
            elif ctype == "cancel_enrollment":
                _set_enrollment_status(runtime_dir, "cancelled", "Enrollment cancelled.")
                result["result"] = {"message": "Enrollment cancelled."}
            elif ctype == "reset_demo_data":
                result["result"] = {"message": "Demo reset is intentionally not destructive in exhibition mode."}
            else:
                raise RuntimeError(f"Unsupported command: {ctype}")
        except Exception as e:
            result["status"] = "failed"
            result["error"] = str(e)
        handled.add(cid)
        _write_command_result(runtime_dir, result)
    _mark_commands_seen(runtime_dir, handled)
...
def main():
    ...
    print("    f  -- enroll from image file or folder")
    ...
    print("    l  -- toggle entry/exit count line")
    ...
    last_raw_frame = None
    last_runtime_state = 0.0
    last_runtime_frame = 0.0
    last_runtime_heartbeat = 0.0
    last_runtime_command_poll = 0.0
    ...
    try:
        while True:
            ...
            try:
                attendance.auto_clock_out_stale(cam_id, cam_location)
                vision.set_attendance_badges(attendance.get_badges())
            except Exception as e:
                print(f"[WARN] Attendance badge/auto-clockout update failed: {e}")
            ...
            for evt in events:
                ...
                if event_type == "DANGEROUS_OBJECT":
                    ...
                    voice(f"Warning. PPE violation detected for {target}.",
                          "WARN", dedup_key=f"ppe:{target}")
                elif event_type == "RUNNING":
                    voice(f"Notice. Running behavior detected for {target}.",
                          "WARN", dedup_key=f"run:{target}")
            ...
            if now_runtime - last_runtime_heartbeat >= 1.0:
                try:
                    _publish_runtime_heartbeat(runtime_dir, "online", "connected", fps, last_runtime_error)
                    last_runtime_heartbeat = now_runtime
                except Exception as e:
                    print(f"[RUNTIME] Heartbeat failed: {e}")

            if now_runtime - last_runtime_state >= 0.35:
                try:
                    _publish_runtime_state(runtime_dir, vision, display, cam_id, cam_location, fps, start_time, events, last_runtime_error)
                    last_runtime_state = now_runtime
                except Exception as e:
                    print(f"[RUNTIME] State publish failed: {e}")

            if now_runtime - last_runtime_frame >= 0.16:
                try:
                    _publish_runtime_frame(runtime_dir, display)
                    last_runtime_frame = now_runtime
                except Exception as e:
                    print(f"[RUNTIME] Frame publish failed: {e}")

            if now_runtime - last_runtime_command_poll >= 0.5:
                try:
                    _process_runtime_commands(runtime_dir, db, attendance, alert_mgr, vision, last_raw_frame, snapshot_dir, cam_id, cam_location)
                    last_runtime_command_poll = now_runtime
                except Exception as e:
                    print(f"[RUNTIME] Command poll failed: {e}")
            ...
            if key == ord('q'):
                ...
                CONFIG["SHOW_COUNT_LINE"] = not CONFIG.get("SHOW_COUNT_LINE", False)
                print(f"[UI] Count line = {CONFIG['SHOW_COUNT_LINE']}")
                ...
                cv2.destroyWindow("Security & Attendance Feed")
                name = input("\n[ENROLL-file] Enter person name (blank = cancel): ").strip()
                if not name:
                    print("[ENROLL-file] Cancelled.")
                else:
                    path = input("[ENROLL-file] Enter image file or folder path: ").strip().strip('"')
                    try:
                        ok = vision.enroll_from_image_path(path, name, db=db)
                        if ok:
                            voice(f"{name} enrolled from image.", "INFO", dedup_key=f"enroll_file:{name}")
                        else:
                            voice("Image enrollment failed.", "WARN", dedup_key="enroll_file_failed")
                    except Exception as e:
                        print(f"[ENROLL-file] Failed: {e}")
        ...
        try:
            try:
                auto_closed = attendance.clock_out_open_sessions_on_shutdown()
                if auto_closed:
                    print(f"[ATTENDANCE] Auto clocked out on shutdown: {auto_closed}")
            except Exception as e:
                print(f"[ATTENDANCE] Shutdown auto clock-out failed: {e}")
