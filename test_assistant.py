...
def _build_system_prompt() -> str:
    return (
        ...
        "9. Suggest follow-up actions when relevant.\n"
        "10. IMPORTANT: When you have enough information to answer, give a clear, natural "
        "response. Do NOT always call tools — if the user says 'hello' or 'thanks', just respond.\n"
...
class AIAssistant:
    def __init__(self, db, alert_manager=None, vision_system=None, config=None):
        ...
        # FIX: separate lock for TTS so other threads can announce too
    ...
    # ------------------------------------------------------------------
    # SPEAKING  (FIX: robust, non-blocking-safe, with timeout)
    # ------------------------------------------------------------------
    def _speak(self, text: str):
        """
        Speak text using TTS. Always prints to console as fallback.
        Uses a dedicated lock so the main thread and listener thread
        don't deadlock.
        """
        if not text or not text.strip():
            return

        tts_engine_name = self.config.get("tts_engine", "pyttsx3")

        # OpenAI TTS (non-blocking, played via subprocess)
        if tts_engine_name == "openai" and self._openai_client:
            self._speak_openai(text)
            return

        # pyttsx3 (runs in-process, needs lock)
        if self._tts_engine is not None:
            with self._tts_lock:
                try:
                    self._tts_engine.stop()
                    self._tts_engine.say(text)
                    # FIX: runAndWait with timeout protection
                    # pyttsx3 doesn't natively support timeout, so we run
                    # it in a thread and enforce a max wait.
                    ...
                    t.start()
                    done_event.wait(timeout=15.0)  # 15-second max
                    if not done_event.is_set():
                        # Timeout – engine is stuck
                        try:
                            self._tts_engine.stop()
                        except Exception:
                            pass
    ...
    # ------------------------------------------------------------------
    # LISTEN LOOP  (FIX: always prints + speaks the answer)
    # ------------------------------------------------------------------
        ...
        try:
            ...
            while not self._stop_event.is_set():
                ...
                if any(cmd in user_text.lower() for cmd in [
                    "exit", "stop", "quit", "done", "that's all",
                    "that is all", "thank you goodbye", "goodbye", "bye",
                ]):
                    farewell = self.config.get(
                        "farewell_message",
                ...
                print(f"\n[USER] {user_text}")
                thinking_msg = self.config.get("thinking_indicator", "[AI] Thinking...")
                print(thinking_msg)

                try:
                    answer = self.ask(user_text)
                    # FIX: ALWAYS print the answer to console, even if TTS fails
                    print(f"[AI] {answer}")
                    self._speak(answer)
                except Exception as e:
    ...
    # ------------------------------------------------------------------
    # CORE QUESTION PROCESSING  (FIX: always returns a usable answer)
    # ------------------------------------------------------------------
    def _process_question(self, question: str) -> str:
        ...
        last_content = None  # Track the last response content

        for _iteration in range(max_iterations):
            try:
                # ADDED: timeout=20.0 to prevent hanging forever
            ...
            # FIX: Save any content the model provided (even mid-tool-call)
            ...
            # FIX: Model gave a final answer (no tool calls)
            final_answer = assistant_message.content
            if not final_answer or not final_answer.strip():
                final_answer = "I processed your request but have no specific response."
        ...
        # FIX: If we exhausted iterations, use last_content or a graceful fallback
        ...
        fallback = (
            "I gathered a lot of information but need to simplify. "
    ...
    def _tool_get_enrolled_faces(self) -> str:
        ...
        for face in faces:
            result.append({
                ...
                "has_thumbnail": face.get("thumbnail_path") is not None,
            })
        return json.dumps({"total": len(result), "faces": result}, default=str)
    ...
    def _tool_get_event_summary(self, days: int) -> str:
        ...
        result = [{"event_type": r["event_type"], "total_count": int(r["total"])} for r in summary]
        return json.dumps({"period_days": days, "summary": result})
    ...
    def _tool_get_events_by_type(self, event_type: str, limit: int) -> str:
        ...
        return json.dumps({"event_type": event_type, "count": len(result), "events": result}, default=str)
    ...
    def _tool_get_person_events(self, person_name: str, limit: int) -> str:
        ...
        return json.dumps({
            "person": person_name, "person_id": person_id,
            "total_events": len(result), "events": result,
    ...
    def _tool_get_person_behavior_profile(self, person_name: str) -> str:
        person_id = self.db.get_person_id(person_name)
        if person_id is None:
            return json.dumps({"error": f"Person '{person_name}' not found in the database."})
        profile = self.db.get_behavior_profile(person_id)
        try:
            type_counts = self.db._fetchall(
                "SELECT event_type, COUNT(*) as cnt FROM events WHERE person_id = ? "
                "GROUP BY event_type ORDER BY cnt DESC", (person_id,))
        except Exception:
            type_counts = []
        event_breakdown = [{"event_type": r["event_type"], "count": r["cnt"]} for r in type_counts]
        return json.dumps({
            "person": person_name,
            "behavior_profile": profile,
    ...
    def _tool_get_alert_history(self, limit: int) -> str:
        ...
        return json.dumps({"count": len(history), "alerts": history}, default=str)
    ...
    def _tool_get_audit_log(self, action: Optional[str], limit: int) -> str:
        ...
        return json.dumps({"count": len(result), "entries": result}, default=str)
    ...
    def _tool_search_events(self, query: str, days: int, limit: int) -> str:
        ...
        try:
            ...
            rows = self.db._fetchall(
                ...
                "WHERE e.timestamp >= ? AND (e.event_type LIKE ? OR e.details_json LIKE ? "
...
if __name__ == "__main__":
    ...
    assistant = AIAssistant(db=db, config={"stt_engine": "text"})
