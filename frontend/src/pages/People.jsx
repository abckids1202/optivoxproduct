      ...
      {selectedId != null && <section className="panel profile-detail-panel">
        ...
        {profile && <>
          ...
          <div className="profile-detail-grid">
            ...
            <div><p className="eyebrow">Automatic attendance gate</p>{(profile.attendance_decisions || []).slice(0, 6).map((decision) => <div className="detail-row" key={decision.id}><strong>{decision.decision}</strong><span>{decision.reason || "No reason"} · frame {decision.source_frame_id ?? "-"}</span></div>)}{!profile.attendance_decisions?.length && <p className="empty-copy">No attendance gate decisions.</p>}</div>
          </div>
        </>}
      </section>}
    </div>
